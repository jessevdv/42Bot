...
class friends(commands.Cog):
  ...
  async def friends(self, ctx: commands.Context):
    ...
    error_embed = discord.Embed(title = "**An error occured!** Chech if the intra_id is valid!", color=color_err)
        ...
        try:
          ...
          for friend in friendlist:
            ...
            if location:
              ...
              row = location[3]
              seat = location[5:]
              location_data = {"floor":floor, "row":row, "seat":seat}
              locations.append({"name":friend, "location":location_data})
